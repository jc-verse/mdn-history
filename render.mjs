...
<p>Only PRs targeting <strong>main</strong> at collection time are included throughout this report. Other target branches are excluded from all historical counts and statistics.</p>
...
<div class="panel"><div id="plot" role="img" aria-label="Number of open issues and pull requests in mdn/content, sampled weekly, every six hours, or hourly depending on the selected range."></div></div>
...
<noscript><p>Enable JavaScript to view the interactive chart.</p></noscript>
</main><script>
...
Plotly.newPlot("plot",${JSON.stringify(series)},${JSON.stringify(layout)},${JSON.stringify(config)})
  ...
  // Older runs left exports in the deployment directory. The report embeds
  // its data, so remove those files before the directory is published again.
