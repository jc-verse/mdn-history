...
test("y-axis starts at zero and fits visible lines on zoom, pan, reset, and legend changes", () => {
  ...
  mount(plot, history, findExtrema, sampleRange, () => {}, (_, changes) => {
    ...
    assert.equal(changes["yaxis.range"][0], 0, "the y-axis must always start at zero");
  ...
  assert.deepEqual(axis(), [0, 1050]);
  zoom("2026-07-01", "2026-08-31");
  assert.deepEqual(axis(), [0, 126], "off-screen peak must not determine the scale");
  show(1, "legendonly");
  assert.deepEqual(axis(), [0, 126], "hiding PRs must preserve the zero baseline");

  zoom("2026-06-30", "2026-08-02");
  assert.deepEqual(axis(), [0, 136.5], "include interpolated values at viewport edges");
  zoom("2026-07-01T06:00:00Z", "2026-07-31T18:00:00Z");
  assert.equal(axis()[0], 0);
  assert.ok(Math.abs(axis()[1] - (110 - 2.5 / 31) * 1.05) < 1e-9,
    "a viewport between weekly samples must fit the line crossing it");
  zoom("2026-07-02", "2026-07-03");
  assert.deepEqual(axis(), [0, 105], "constant intraday counts need a nonzero range");
  show(0, false);
  assert.deepEqual(axis(), [0, 1], "all-hidden data needs a finite fallback");
  show(1, true);
  assert.deepEqual(axis(), [0, 21]);
  show(0, true);
  assert.deepEqual(axis(), [0, 105]);
  zoom("2026-08-02", "2026-08-03");
  assert.deepEqual(axis(), [0, 115.5], "panning should refit the axis");

  listeners.plotly_selected({ range: { x: ["2026-06-01", "2026-06-02"] } });
  assert.ok(Math.abs(axis()[1] - 116.2) < 1e-9,
  ...
  assert.deepEqual(axis(), [0, 1050], "reset restores the full-history scale");
  plot.layout.yaxis.range = [0, 5000];
  plot.layout.yaxis.autorange = true;
  listeners.plotly_relayout({ "yaxis.autorange": true });
  assert.deepEqual(axis(), [0, 1050], "autoscale refits even when the date range is unchanged");
});
